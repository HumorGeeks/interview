...
PS：引入链表，采用头插法解决哈希冲突问题

备注：

- 整形值HashCode为自身，采用整形值作为key可以有效地防止哈希碰撞
- 采用位运算获取索引值，效率最高

### put数据的时候如何扩容

1. 检查是否有重复的键，如果有，则将原来的进行替换
2. threshold扩容阈值 = CAPACITY （初始化的数组总长度）* LOAD_FACTOR（加载因子）
3. 扩容之后容量还是之前的2倍（2^N）
4. 原来的元素如何迁移
   1. 遍历链表里边的所有元素和每个元素对应的链表；
   2. 根据新的哈希值和新的容量计算该entry应该存放的数组下标位置，**迁移过程中原链表在数组中的位置发生改变**（rehash）
   3. **原数组中的元素因为使用头插法所以顺序倒置**
      - 记录oldhash表中e.next
      - rehash计算出数组的位置(hash表中桶的位置)
      - 先将e.next指向new hash表中的第一个元素
